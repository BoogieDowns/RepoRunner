    ...
    >
      ...
      <div
        style={{
          ...
          background: `linear-gradient(to bottom,
            transparent 0%,
            rgba(136,6,14,${op * 0.03}) 10%,
            rgba(152,6,18,${op * 0.12}) 32%,
            rgba(178,8,22,${op * 0.34}) 58%,
            rgba(198,12,26,${op * 0.64}) 80%,
            rgba(208,12,28,${op * 0.88}) 100%
          )`,
        }}
      />
      ...
      <div
        style={{
          ...
          height: `${headH}px`,
          background: `linear-gradient(to bottom,
            rgba(195,12,26,${op * 0.80}),
            rgba(245,16,38,${op * 0.97}),
            rgba(248,52,72,${op * 0.50})
          )`,
          boxShadow: glowMul > 0
            ? `0 0 ${glowPx}px ${glowSpr}px rgba(230,14,34,${op * glowMul})`
            : "none",
...
function AmbientBackground({
  ...
  const moonBorder = anyRunning
    ? bothRunning ? "rgba(162,10,22,0.20)" : "rgba(140,8,18,0.13)"
    : "rgba(88,4,12,0.06)";
  const moonGlow = anyRunning
    ? bothRunning
      ? "0 0 60px rgba(162,10,22,0.18), 0 0 120px rgba(120,6,16,0.10)"
      : "0 0 40px rgba(140,8,18,0.11)"
    : "none";
      ...
      {ripples.flatMap(r => [
        ...
        <svg
          ...
          style={{
            ...
            overflow:  "hidden",
            pointerEvents: "none",
            filter:    `drop-shadow(0 0 3px rgba(196,12,26,${r.op * 0.42}))`,
        ...
        >
          ...
          <path
            d={WAVE_PATH}
            fill="none"
            stroke={`rgba(185,14,28,${r.op * 0.28})`}
            strokeWidth="5"
            strokeLinecap="round"
          />
          {/* Core line */}
          <path
            d={WAVE_PATH}
            fill="none"
            stroke={`rgba(222,14,32,${r.op * 0.65})`}
            strokeWidth="1.5"
            strokeLinecap="round"
          />
        </svg>,
        ...
        <div
          ...
          style={{
            ...
            borderRadius: "50%",
            transform:    "translateX(-50%)",
            background:   `rgba(248,130,148,${r.op * 0.55})`,
            boxShadow: [
              `0 0  3px 1px rgba(240, 16, 38,${r.op * 0.48})`,
              `0 0  7px 2px rgba(195, 10, 26,${r.op * 0.30})`,
              `0 0 13px 4px rgba(155,  6, 18,${r.op * 0.14})`,
            ].join(", "),
            pointerEvents:   "none",
  ...
  */
        ...
        return (
          <span key={lightKey} style={{
            ...
            <span style={{
              ...
              boxShadow: "inset 0 1px 2px rgba(0,0,0,0.45), inset 0 -1px 1px rgba(0,0,0,0.28), inset 1px 0 1px rgba(0,0,0,0.18), inset -1px 0 1px rgba(0,0,0,0.18), 0 0 15px rgba(185,14,28,0.82), 0 0 5px rgba(185,14,28,0.56)",
            }}>
  ...
  return (
    <div className="h-screen flex flex-col bg-background text-foreground animate-in fade-in duration-300 relative overflow-hidden">
      ...
      {/* ── Header ── */}
      <header
        className="h-[72px] flex-none flex items-center px-5 gap-4 relative"
        style={{ borderBottom: "1px solid #161616", background: "rgba(7,7,7,0.92)" }}
      >
        {/* Brand */}
        <div
          className="flex items-center gap-2 flex-none"
          style={{ filter: "drop-shadow(0 0 10px rgba(185,14,28,0.14))" }}
        >
          <RepoRunnerLogo />
        </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
            ...
            <Card
              ...
              style={{
                ...
                boxShadow: "0 4px 24px rgba(0,0,0,0.74), 0 0 0 1px rgba(185,14,28,0.05), inset 0 1px 0 rgba(185,14,28,0.07)",
              }}
            >
            ...
            <div>
              {logs.map((log) => (
                <div
                  ...
                  onMouseEnter={(e) => { (e.currentTarget as HTMLDivElement).style.background = "rgba(185,14,28,0.025)"; }}
